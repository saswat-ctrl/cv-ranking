"""
Standalone CV Ranking Test - No Database Required
Tests the ranking system with actual files from test_documents
"""
import asyncio
import os
import sys
...
CV_FILES = [
    "Anish Sinha CV'25.pdf",  # Note: This has a special quote character
...
async def main():
    try:
        ...
        cv_embeddings_results = generate_embeddings_batch(cv_texts)
        
        # Unpack results (each is a tuple of (bytes, error))
        cv_embeddings = [result[0] for result in cv_embeddings_results]
        ...
        for idx, c in enumerate(ranked, 1):
            ...
            print(f"       💡 {c['reasoning']}")
            print(f"       🎯 Matched Skills: {', '.join(c['matching_skills'][:5])}" + (" ..." if len(c['matching_skills']) > 5 else ""))
            print("-"*100)
